...
for key in keys:
	...
	#with open('summary_by_date-'+key+'.json','r') as read:
	#	summary[key] = json.load(read)
...
daily_change, d,GGA_opt,GGA_sp,hGGA_opt,hGGA_sp, ref = [],[],[],[],[],[],[]
...
for item in traj:
	if item not in single_Al:
		...
		if ads == 'Pd1':
			...
			if traj[item]['hGGA']['opt']['running'] != 0 or traj[item]['hGGA']['opt']['complete'] + traj[item]['hGGA']['opt']['failed'] < total[ads]:
				'hGGA opt is running'
				...
				if traj[item]['hGGA']['sp']['running'] == 0 and traj[item]['hGGA']['sp']['complete'] == 0 and traj[item]['hGGA']['sp']['failed'] == 0:
					...
					hGGA_sp.append(item)
				elif traj[item]['hGGA']['sp']['running'] != 0 or traj[item]['hGGA']['sp']['complete'] + traj[item]['hGGA']['sp']['failed'] < 5:
					'hGGA sp is running'
					empty.append(item)
				elif traj[item]['hGGA']['sp']['complete'] > 4:
					'completed'
