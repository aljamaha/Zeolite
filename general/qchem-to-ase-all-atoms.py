...
Input format: python strip-qchem-output.py <qchem-output filename> <xyz initial input filename> <total #of qm atoms>
...
os.system('mkdir test')	#data are saved here
...
for index, line in enumerate(lines):
	...
	if 'Standard Nuclear Orientation' in line:
		if ' ----------------------------------------------------------------' in lines[index+int(atoms)+3]:
			file_number +=1
			g = open('test/'+str(file_number)+'.xyz','w')
			g.write(atoms+'\n\n')
			for i in range(index,index+int(atoms)+3):
				if i in [index,1+index,2+index]:
					...
					g.write(lines[i][11:]+'\n')
			g.close()
os.system('cp test/'+str(file_number)+'.xyz final.xyz')
