...
data_dir    = '/home/aljama/CHA-Pd2/data/'			#dir where json data are saved
calc_dir    = '/home/aljama/CHA-Pd2/calculations/'		#dir where calculations are done
results_dir = '/home/aljama/CHA-Pd2/results-analysis/' 	#dir where results are to be saved
H_data      = '/home/aljama/CHA-Pd2-H/data/'	#dir where data for H adsorptions sites are saved
zeolite     = 'CHA'
...
if sorted_plot == True:
	...
	for index, item in enumerate(new_E):	
		if new_x[index] in [52, 30]:
			colour = 'b'
		elif new_x[index] in [29]:
			colour = 'g'
		elif new_x[index] in [39,65] :
			colour = 'r'
		else:
			colour = 'c'
		plt.bar(x_pts[index], new_E[index], color=colour, align='center', alpha=1)

		Pd2[new_x[index]] = new_E[index]

	if zeolite == 'CHA':
		new_x = ['6MR (NNNN)','6MR (NNN)','6MR + 4MR (NNNN)', '6MR + 6MR (NNNN)', '6MR + 4MR (NNNN)','8 MR (NNN)','6MR + 4MR (NNNN)','4MR (NNN)','8MR (NNNN)']	
	plt.xticks(x_pts, new_x, rotation = 90)
	plt.ylabel('Energy (eV)',fontsize=12)

from matplotlib.lines import Line2D
custom_lines = [Line2D([0], [0], color='b', lw=6),
                Line2D([0], [0], color='c', lw=6),
		Line2D([0], [0], color='r', lw=6),
                Line2D([0], [0], color='g', lw=6)]
plt.legend(custom_lines, ['6MR', 'Al-Al not in same MR', '8MR', '4MR'],loc=4)

print('Pd2 energy dict:', Pd2)
print('Raw Pd2 energy:', Pd2_raw)
...
'''
	...
	plt.xticks(x_min, x_min, rotation=90,fontsize=12)
